...
router3.get("/", async (c) => {
  ...
  const [totalRes, rows] = await Promise.all([
    ...
    drizzleDb(c).select().from(donors).where(whereClause).orderBy(orderByCol).limit(limitNum).offset(offset)
  ]);
  return jsonOk(c, {
    items: rows,
...
// .wrangler/tmp/bundle-Q5hJhZ/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Q5hJhZ/middleware-loader.entry.ts
