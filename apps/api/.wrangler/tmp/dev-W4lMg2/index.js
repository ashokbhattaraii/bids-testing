...
router3.get("/", async (c) => {
  ...
  const offset = (pageNum - 1) * limitNum;
  const [totalRes, rowsRes] = await Promise.all([
    db(c).prepare(`SELECT COUNT(*) as total FROM donors ${where}`).bind(...binds).first(),
    db(c).prepare(`SELECT * FROM donors ${where} ORDER BY ${orderBy} LIMIT ?${i} OFFSET ?${i + 1}`).bind(...binds, limitNum, offset).all()
  ]);
  return jsonOk(c, {
...
router3.get("/:id", async (c) => {
  const row = await db(c).prepare("SELECT * FROM donors WHERE id = ?1").bind(c.req.param("id")).first();
...
router3.get("/lookup/by-phone", async (c) => {
  ...
  const row = await db(c).prepare("SELECT id, name, phone FROM donors WHERE phone = ?1 LIMIT 1").bind(phone.trim()).first();
  return jsonOk(c, row ?? null);
});
...
router3.post("/", zValidator("json", createSchema2), async (c) => {
  const body = c.req.valid("json");
  const id = newId();
  console.log("Creating donor with data:", body, "Generated ID:", id);
  ...
  const created = await db(c).prepare("SELECT * FROM donors WHERE id = ?1").bind(id).first();
...
router3.put("/:id", zValidator("json", updateSchema2), async (c) => {
  const id = c.req.param("id");
  const body = c.req.valid("json");
  const existing = await db(c).prepare("SELECT id, donation_count, last_donation FROM donors WHERE id = ?1").bind(id).first();
  ...
  if ("name" in body && body.name) {
    sets.push(`name = ?${i++}`);
    binds.push(capitalize2(body.name));
  }
  for (const [key, col] of Object.entries(fieldMap)) {
    if (key in body) {
      sets.push(`${col} = ?${i++}`);
  ...
  const updated = await db(c).prepare("SELECT * FROM donors WHERE id = ?1").bind(id).first();
  return jsonOk(c, updated, "Donor updated");
});
router3.delete("/:id", requireRole("admin"), async (c) => {
  const id = c.req.param("id");
  const existing = await db(c).prepare("SELECT id FROM donors WHERE id = ?1").bind(id).first();
  if (!existing) return jsonError(c, 404, "Donor not found");
...
router3.post(
  ...
  async (c) => {
    ...
    const body = c.req.valid("json");
    const existing = await db(c).prepare("SELECT id FROM donors WHERE id = ?1").bind(donorId).first();
    if (!existing) return jsonError(c, 404, "Donor not found");
    ...
    await db(c).prepare(`UPDATE donors SET last_contacted = datetime('now'), updated_at = datetime('now') WHERE id = ?1`).bind(donorId).run();
...
// .wrangler/tmp/bundle-RJPA6y/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-RJPA6y/middleware-loader.entry.ts
