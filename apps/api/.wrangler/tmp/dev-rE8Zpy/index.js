...
// .wrangler/tmp/bundle-0tdl3R/strip-cf-connecting-ip-header.js
...
// ../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260506.1/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-console
...
// ../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260506.1/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-process
...
function jsonError(c, status, message2) {
  return c.json({ success: false, message: message2 }, status);
}
__name(jsonError, "jsonError");
function jsonOk(c, data, message2, status = 200) {
  return c.json({ success: true, data, ...message2 ? { message: message2 } : {} }, status);
}
...
async function signJwt(user, secret) {
  return new SignJWT({ ...user }).setProtectedHeader({ alg: ALGORITHM }).setIssuedAt().setExpirationTime(EXPIRY).sign(secretKey(secret));
}
__name(signJwt, "signJwt");
...
router.post("/login", zValidator("json", loginSchema), async (c) => {
  ...
  const row = await db(c).prepare("SELECT id, name, email, password_hash, role, is_active, avatar FROM users WHERE email = ?1 LIMIT 1").bind(email.toLowerCase()).first();
  if (!row) {
  ...
  return jsonOk(c, {
    token,
    user: { ...user, avatar: row.avatar ?? void 0 }
  });
});
router.get("/me", requireAuth, (c) => {
  return jsonOk(c, c.var.user);
});
var seedSchema = external_exports.object({ password: external_exports.string().min(8) });
router.post("/seed", zValidator("json", seedSchema), async (c) => {
  const { password } = c.req.valid("json");
  const existing = await db(c).prepare("SELECT password_hash FROM users WHERE id = ?1").bind("usr_seed_admin_001").first();
  if (!existing) {
  ...
  await db(c).prepare("UPDATE users SET password_hash = ?1, updated_at = datetime('now') WHERE id = ?2").bind(hash, "usr_seed_admin_001").run();
...
router.post("/change-password", requireAuth, zValidator("json", changePasswordSchema), async (c) => {
  ...
  const { id } = c.var.user;
  const row = await db(c).prepare("SELECT password_hash FROM users WHERE id = ?1").bind(id).first();
  if (!row?.password_hash) {
...
router.post("/google/token", zValidator("json", googleTokenSchema), async (c) => {
  ...
  if (!row) {
    return jsonError(c, 403, "not_authorized");
  }
  if (!row.is_active) {
...
router2.put("/:id", zValidator("json", updateSchema), async (c) => {
  ...
  if (sets.length === 0)
    return jsonError(c, 400, "No fields to update");
...
router2.delete("/:id", requireRole("admin", "call_operator"), async (c) => {
  const id = c.req.param("id");
...
router2.post(
  ...
  async (c) => {
    ...
    if (!reminder)
      return jsonError(c, 404, "No pending follow-up reminder found");
    await db(c).prepare(
...
router3.get("/:id", async (c) => {
  ...
  if (!row)
    return jsonError(c, 404, "Donor not found");
  return jsonOk(c, row);
});
router3.get("/lookup/by-phone", async (c) => {
  const { phone } = c.req.query();
  if (!phone)
    return jsonError(c, 400, "phone query param required");
...
router4.get("/:id", async (c) => {
  ...
  if (!row)
    return jsonError(c, 404, "Pledge not found");
  return jsonOk(c, row);
});
...
router4.post("/:id/convert", requireRole("admin", "call_operator"), async (c) => {
  const id = c.req.param("id");
...
async function withInventory(c, hospital) {
  ...
  for (const row of inv.results ?? [])
    bloodInventory[row.blood_type] = row.units;
...
router5.get("/:id", async (c) => {
  ...
  if (!row)
    return jsonError(c, 404, "Hospital not found");
...
router5.put("/:id", requireRole("admin", "call_operator"), zValidator("json", updateSchema4), async (c) => {
  const id = c.req.param("id");
  const body = c.req.valid("json");
...
router6.put("/:id", requireRole("admin", "call_operator"), zValidator("json", updateSchema5), async (c) => {
  const id = c.req.param("id");
  const { status } = c.req.valid("json");
  const existing = await db(c).prepare("SELECT id FROM feedback WHERE id = ?1").bind(id).first();
  if (!existing)
    return jsonError(c, 404, "Feedback not found");
...
var createSchema6 = external_exports.object({
  ...
  role: external_exports.enum(["admin", "call_operator", "volunteer"]).default("volunteer"),
  isActive: external_exports.boolean().default(true)
});
router7.post("/", zValidator("json", createSchema6), async (c) => {
  const body = c.req.valid("json");
  const exists = await db(c).prepare("SELECT id FROM users WHERE email = ?1").bind(body.email.toLowerCase()).first();
  if (exists)
    return jsonError(c, 409, "A user with this email already exists");
  const id = newId();
  const hash = await hashPassword(body.password);
  await db(c).prepare(
    `INSERT INTO users (id, name, email, password_hash, role, is_active)
       VALUES (?1,?2,?3,?4,?5,?6)`
  ).bind(id, body.name, body.email.toLowerCase(), hash, body.role, body.isActive ? 1 : 0).run();
...
var updateSchema6 = external_exports.object({
  name: external_exports.string().min(1).optional(),
  role: external_exports.enum(["admin", "call_operator", "volunteer"]).optional(),
...
router8.put(
  ...
  async (c) => {
    ...
    if (!existing)
      return jsonError(c, 404, "Unverified donor not found");
...
router11.get("/", async (c) => {
  ...
  if (!requestId)
    return jsonError(c, 400, "requestId query param required");
  const rows = await db(c).prepare(
    `SELECT rd.id as assignment_id, rd.status as assignment_status,
              rd.notes as assignment_notes, rd.assigned_at,
...
router11.post(
  ...
  async (c) => {
    ...
    if (!request)
      return jsonError(c, 404, "Blood request not found");
    ...
    if (duplicate)
      return jsonError(c, 409, "Donor is already assigned to this request");
    const id = newId();
    await db(c).prepare(
...
router11.put(
  ...
  async (c) => {
    ...
    if (!existing)
      return jsonError(c, 404, "Assignment not found");
    const sets = [`status = ?1`];
...
// ../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260506.1/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// ../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260506.1/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-0tdl3R/middleware-insertion-facade.js
...
// ../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260506.1/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-0tdl3R/middleware-loader.entry.ts
