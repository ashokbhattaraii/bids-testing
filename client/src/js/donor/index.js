...
$(document).ready(function() {
  let list = new DonorList({ target: "#tblDonor" });
  ...
  $("#filterByGroup").change(e => {
    ...
    if (value.length > 0) {
      ...
      list.table.load(`/api/v1/donors?group=${encodeURIComponent(value)}`);
    }
  });
  ...
  const filter = (name, value) => {
    ...
    if (value.length > 2) {
      ...
      list.table.load(`/api/v1/donors?${name}=${encodeURIComponent(value)}`);
    }
  };
});
